...
public class SymTabCreatorVisitor implements Visitor{
    ...
    public void visit(ClassDeclNode node) {
        ...
        if(similarEntry==null){
            ...
            out.addSemanticError(Integer.valueOf(linenum), String.format("Duplicate class definition. %s is defined at line %s", similarEntry.name, similarEntry.linenum));
        ...
        for(Node inherIdNode : childs.get(1).getChildren()){
            ...
            if(inherClassEntry!=null){
                ...
                out.addSemanticError(Integer.valueOf(inherIdNode.linenum), String.format("Inherited class %s is not defined", inheridname));
            }
        }
        ...
        for(SymTabEntry membEntry : membListTable.entries){
            if(membEntry.kind==Kind.Var){
                ...
                if(similarEntry!=null){//var hiding
                    out.addSemanticWarning(Integer.valueOf(membEntry.linenum), String.format("Variable %s is hiding the one declared at %s", membEntry.name, similarEntry.linenum));
                }
            }
            if(membEntry.kind==Kind.Func){
                ...
                if(similarEntry!=null){//overiden function
                    out.addSemanticWarning(Integer.valueOf(membEntry.linenum), String.format("Function %s is overiding the one declared at %s", funcEntry.stringRepr(), similarEntry.linenum));
                }
                else{//check if overloading
                    similarEntry = localtable.search(funcEntry.name, Kind.Func);
                    if(similarEntry!=null){
                        out.addSemanticWarning(Integer.valueOf(membEntry.linenum), String.format("Function %s is an overload", funcEntry.stringRepr()));
                    }
                }
            }
    ...
    public void visit(FuncBodyNode node) {
        ...
        for(SymTabEntry varEntry : varDeclListTable.entries){
            ...
            if(similarEntry!=null){
                out.addSemanticError(Integer.valueOf(varEntry.linenum), String.format("Duplicate Variable. %s is already declared at %s", varEntry.name, similarEntry.linenum));
            }
            else{
    ...
    public void visit(FuncDefNode node) {
        ...
        for(VarEntry paramentry : fparamlist){
            ...
            if(similarentry==null){
                ...
                out.addSemanticError(Integer.valueOf(paramentry.linenum), String.format("Duplicate function param name %s", paramentry.name));
            }
        }
        ...
        if(scopeSpec!=null){//class member function
            ...
            if(classentry==null){//function's class scope doesn't exist
                out.addSemanticError(Integer.valueOf(linenum), String.format("Class %s for member function isn't defined.", scopeSpec));
            }
            else{
                //check if function was declared
                FuncEntry funcentry = (FuncEntry) classentry.scope.searchFunclocal(funcname, fparamlist);
                if(funcentry==null){
                    out.addSemanticError(Integer.valueOf(linenum), String.format("Definition provided for undeclared function in class %s", classentry.name));
                }
                else{
                    //check if return type matches the one declared
                    if(!funcentry.type.equals(type)){
                        out.addSemanticError(Integer.valueOf(linenum), String.format("Function definition return type %s does not match %s declared at line %s", type, funcentry.type, funcentry.linenum));
                    }
                    else{
                ...
                for(SymTabEntry varentry : functable.entries){//entries can be var or param, but can only hide var members from class
                    ...
                    if(similarentry!=null){
                        out.addSemanticWarning(Integer.valueOf(varentry.linenum), String.format("%s %s is hiding variable declared at line %s", varentry.kind, varentry.name, similarentry.linenum));
                    }
                }
            }
        }
            ...
            if(funcentry!=null){
                out.addSemanticError(Integer.valueOf(linenum), String.format("Duplicate function definition %s. Function already defined at %s", funcentry.stringRepr(), funcentry.linenum));
            }
            else{
                ...
                if(globaltable.searchlocal(funcname, Kind.Func)!=null){
                    out.addSemanticWarning(Integer.valueOf(linenum), String.format("Function %s is an overload", funcentry.stringRepr()));
                }
        ...
        for (Node child : node.getChildren()) {
            ...
            if(membentry.kind==Kind.Var){
                ...
                if(similarentry!=null){//duplicate
                    out.addSemanticError(Integer.valueOf(membentry.linenum), String.format("Duplicate variable declaration. %s was already defined at line %s", similarentry.name, similarentry.linenum));
                }
                else{
                    localtable.addEntry(membentry);
                }
            ...
            if(membentry.kind==Kind.Func){
                ...
                if(similarEntry!=null){
                    out.addSemanticError(Integer.valueOf(membentry.linenum), String.format("Duplicate function declaration. %s was already declared at line %s", funcentry.stringRepr(), similarEntry.linenum));
                }
                else{
    ...
    public void visit(ProgNode node) {
        ...
        //check if all func entries in class tables have a link to a SymTab
        for(SymTabEntry entry : node.symtab.entries){
            //only check for class entries
            if(entry.kind==Kind.Class){
                SymTab classtable = entry.scope;
                for(SymTabEntry classtableEntry : classtable.entries){
                    if(classtableEntry.kind==Kind.Func && classtableEntry.scope==null){
                        out.addSemanticError(Integer.valueOf(classtableEntry.linenum), String.format("Declared member function is not defined"));
                    }
                }
            }
        }
    ...
    public void visit(VarDeclListNode node) {
        ...
        for (Node child : node.getChildren()) {
            ...
            SymTabEntry similarentry = localtable.searchlocal(varentry.name, Kind.Var);
                if(similarentry!=null){//duplicate
                    out.addSemanticError(Integer.valueOf(varentry.linenum), String.format("Duplicate variable declaration. %s was already defined at line %s", similarentry.name, similarentry.linenum));
                }
                else{
    ...
    public void visit(VarDeclNode node) {
        ...
        for(Node dim : childs.get(2).getChildren()){
            ...
            if(dim.data!=null){//because could be epsilon node, which is not allowed for varDecl
                ...
                out.addSemanticError(Integer.valueOf(linenum), String.format("Declaration of array variable % must specify all its dimensions' sizes", varname));

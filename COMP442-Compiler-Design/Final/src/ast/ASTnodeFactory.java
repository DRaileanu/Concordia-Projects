...
public class ASTnodeFactory {
    ...
    public static Node makeNode(NodeType type){
        Node node = null;
        switch(type){
    ...
    public static Node makeNode(NodeType type, Token token){
        Node node = null;
        switch(type){
    ...
    public static Node makeFamily(NodeType type, Node... childs){
        for(int i=0; i<childs.length-1; i++){
            childs[i].makeSiblings(childs[i+1]);
        }
        Node parent = makeNode(type);

...
public class Parser {
    ...
    public Stack<Pointer<ASTnode>> semStack;
    public ASTnode ast;
    ...
    public Parser(Lexer lex){
        ...
        debug = false;
        success = true;
        semStack = new Stack<Pointer<ASTnode>>();
        errorMessages = new ArrayList<String>();
        derivations = new ArrayList<String>();
        derivTemp = new ArrayList<String>();
        derivIndex = 0;
    }

    
    public boolean parse(){
        Pointer<ASTnode> astP = new Pointer<ASTnode>();
        semStack.push(astP);
        lookahead = nextToken();
        ...
        if(START() & match(TokenType.EOF)){
            ...
            derivations.add(String.join(" ", derivTemp));
            if(success){
                errorMessages.add(0, "Parsing was succesfull, no errors detected");
            }
            else{
                errorMessages.add(0, "Parsing was succesfull, but there were still some errors detected:");
            }
            return true;
        }
        else{
            derivations.add(String.join(" ", derivTemp));
            errorMessages.add(0, "Parsing failed. The following errors were encountered:");
            return false;
        }
    }
    ...
    private boolean match(TokenType tokenType){
        ...
        if(lookahead.getType().equals(tokenType)){
            ...
            success = false;
            errorMessages.add("Unexpected Token "+lookahead.getType()+" at line: "+(lookahead.getLocation())+"; Expected "+tokenType.toString());
            if(debug){
    ...
    private boolean skipErrors(EnumSet<TokenType> first, EnumSet<TokenType> follow){
        ...
        if(
            ...
            success = false;
            errorMessages.add("Unexpected Token "+lookaheadType+" at line: "+(lookahead.getLocation()));
            if(debug){
    ...
    private boolean AParams(){
        ...
        if(first(Symbol.Expr).contains(lookahead.getType())){
            Pointer<ASTnode> exprP = new Pointer<ASTnode>();
            Pointer<ASTnode> nestedExprP = new Pointer<ASTnode>();
    ...
    private boolean ArithExpr(){
        ...
        if(first(Symbol.Term).contains(lookahead.getType())){
            Pointer<ASTnode> leftTermP = new Pointer<ASTnode>();
            Pointer<ASTnode> addOpP = new Pointer<ASTnode>();
            Pointer<ASTnode> arithExprP = new Pointer<ASTnode>();
    ...
    private boolean ArithExprTail(){
        ...
        derivTemp.remove(derivIndex);
        Pointer<ASTnode> addOpP = semStack.pop();
        Pointer<ASTnode> arithExprP = semStack.pop();
        ...
        if(first(Symbol.AddOp).contains(lookahead.getType())){
            Pointer<ASTnode> termP = new Pointer<ASTnode>();
            Pointer<ASTnode> nestedAddOpP = new Pointer<ASTnode>();
            Pointer<ASTnode> nestedArithExprP = new Pointer<ASTnode>();
    ...
    private boolean ArraySizeRept(){
        ...
        if(lookahead.getType().equals(TokenType.lsqbra)){
            Pointer<ASTnode> nestedIntNumP = new Pointer<ASTnode>();
    ...
    private boolean AssignStatTail(){
        ...
        if(first(Symbol.AssignOp).contains(lookahead.getType())){
            Pointer<ASTnode> assignP = new Pointer<ASTnode>();
            Pointer<ASTnode> exprP = new Pointer<ASTnode>();
            semStack.push(exprP);
    ...
    private boolean ClassDecl(){
        ...
        if(lookahead.getType().equals(TokenType.Class)){
            ASTnode classDecl = ASTnodeFactory.makeNode(NodeType.classDecl);
            result.ref.adoptChildren(classDecl);
            Pointer<Token> idToken = new Pointer<Token>();
            Pointer<ASTnode> inherListP = new Pointer<ASTnode>();
            inherListP.ref = ASTnodeFactory.makeNode(NodeType.inherList);
            Pointer<ASTnode> membDeclListP = new Pointer<ASTnode>();
            ...
            if(match(TokenType.Class) & match(TokenType.id, idToken) & Inherit() & match(TokenType.lcurbra) & ClassDeclBody() & match(TokenType.rcurbra) & match(TokenType.semicol) & ClassDecl()){
                ...
                ASTnode id = ASTnodeFactory.makeNode(NodeType.id, idToken.ref);
    ...
    private boolean ClassDeclBody(){
        ...
        if(first(Symbol.Visibility).contains(lookahead.getType()) || first(Symbol.MemberDecl).contains(lookahead.getType())){
            ASTnode membDecl = ASTnodeFactory.makeNode(NodeType.membDecl);
            result.ref.adoptChildren(membDecl);      
            Pointer<ASTnode> visibilityP = new Pointer<ASTnode>();
            Pointer<ASTnode> varDeclORfuncDeclP = new Pointer<ASTnode>();
            semStack.push(result);
    ...
    private boolean Expr(){
        ...
        if(first(Symbol.ArithExpr).contains(lookahead.getType())){
            Pointer<ASTnode> leftArithExprP = new Pointer<ASTnode>();
            Pointer<ASTnode> relOpP = new Pointer<ASTnode>();
            Pointer<ASTnode> rightArithExprP = new Pointer<ASTnode>();
    ...
    private boolean ExprTail(){
        ...
        derivTemp.remove(derivIndex);
        Pointer<ASTnode> relOpP = semStack.pop();
        Pointer<ASTnode> arithExprP = semStack.pop();
    ...
    private boolean FParams(){
        ...
        if(first(Symbol.Type).contains(lookahead.getType())){
            Pointer<Token> idToken = new Pointer<Token>();
            Pointer<ASTnode> typeP = new Pointer<ASTnode>();
            Pointer<ASTnode> dimP = new Pointer<ASTnode>();
            Pointer<ASTnode> nestedFParamP = new Pointer<ASTnode>();
            ...
            if(Type() & match(TokenType.id, idToken) & ArraySizeRept() & FParamsTail()){
                ...
                ASTnode id = ASTnodeFactory.makeNode(NodeType.id, idToken.ref);
                ASTnode dimList = ASTnodeFactory.makeNode(NodeType.dimList);
                if(dimP.ref!=null){
                    dimList.adoptChildren(dimP.ref);
                }
                ASTnode fParam = ASTnodeFactory.makeFamily(NodeType.fParam, typeP.ref, id, dimList);
    ...
    private boolean Factor(){
        ...
        if(first(Symbol.FuncOrVar).contains(lookahead.getType())){
            ...
            Pointer<ASTnode> factorP = new Pointer<ASTnode>();
            ...
            Pointer<ASTnode> expr1P = new Pointer<ASTnode>();
            Pointer<ASTnode> expr2P = new Pointer<ASTnode>();
            Pointer<ASTnode> expr3P = new Pointer<ASTnode>();
            semStack.push(expr3P);
            semStack.push(expr2P);
            semStack.push(expr1P);

            derivTemp.add(derivIndex, "'qm'"); derivTemp.add(derivIndex+1, "'['"); derivTemp.add(derivIndex+2, "<Expr>"); derivTemp.add(derivIndex+3, "':'"); derivTemp.add(derivIndex+4, "<Expr>"); derivTemp.add(derivIndex+5, "']'");
            derivations.add(String.join(" ", derivTemp));
            if(match(TokenType.qm) & match(TokenType.lsqbra) & Expr() & match(TokenType.colon) & Expr() & match(TokenType.rsqbra)){
    ...
    private boolean FuncBody(){
        ...
        if(lookahead.getType().equals(TokenType.lcurbra)){
            ...
            if(match(TokenType.lcurbra) & MethodBodyVar() & StatementList() & match(TokenType.rcurbra)){
                ...
                ASTnode statList = ASTnodeFactory.makeNode(NodeType.statList);
    ...
    private boolean FuncDecl(){
        ...
        if(lookahead.getType().equals(TokenType.Func)){
            Pointer<Token> idToken = new Pointer<Token>();
            Pointer<ASTnode> fParamListP = new Pointer<ASTnode>();
            fParamListP.ref = ASTnodeFactory.makeNode(NodeType.fParamList);
            Pointer<ASTnode> fTypeP = new Pointer<ASTnode>();
            ...
            if(match(TokenType.Func) & match(TokenType.id, idToken) & match(TokenType.lpar) & FParams() & match(TokenType.rpar) & match(TokenType.colon) & FuncDeclTail() & match(TokenType.semicol)){
                ...
                ASTnode id = ASTnodeFactory.makeNode(NodeType.id, idToken.ref);
                result.ref = ASTnodeFactory.makeFamily(NodeType.funcDecl, id, fParamListP.ref, fTypeP.ref);
                return true;
            }
            else return false;
        }
    ...
    private boolean FuncDef(){
        ...
        if(first(Symbol.Function).contains(lookahead.getType())){
            Pointer<ASTnode> funcDefP = new Pointer<ASTnode>();
    ...
    private boolean FuncHead(){
        //*result is funcDef. Adds all required childs (except statBlock)
        /*
        ...
        */
        ...
        if(lookahead.getType().equals(TokenType.Func)){
            Pointer<Token> idToken = new Pointer<Token>();
            Pointer<ASTnode> scopeSpecP = new Pointer<ASTnode>();
            Pointer<ASTnode> fParamListP = new Pointer<ASTnode>();
            fParamListP.ref = ASTnodeFactory.makeNode(NodeType.fParamList);
            Pointer<ASTnode> typeP = new Pointer<ASTnode>();
            semStack.push(typeP);
    ...
    private boolean FuncOrAssignStat(){
        //*result is null, becomes assignStat or var with 1+ childs dataMember/fCall(semantically fCall as last child is fCall)
        //note that assign stat first child is Expr, and second will be var
        //if *result still null after FuncOrAssignStatIdnest(), then *result is var
        ...
        if(lookahead.getType().equals(TokenType.id)){
            Pointer<Token> idToken = new Pointer<Token>();
            Pointer<ASTnode> varElementP = new Pointer<ASTnode>();
            ...
            if(match(TokenType.id, idToken) & FuncOrAssignStatIdnest()){
                ...
                ASTnode var = ASTnodeFactory.makeNode(NodeType.var);
                var.adoptChildren(varElementP.ref);//and all its siblings
                if(result.ref==null){
                    result.ref = var;
                }
                else{
                    result.ref.adoptChildren(var);
                }
    ...
    private boolean FuncOrAssignStatIdnest(){
        ...
        derivTemp.remove(derivIndex);
        Pointer<ASTnode> result = semStack.pop();
        Pointer<ASTnode> varElementP = semStack.pop();
        ...
        if(first(Symbol.IndiceRep).contains(lookahead.getType()) || first(Symbol.FuncOrAssignStatIdnestVarTail).contains(lookahead.getType())){
            Pointer<ASTnode> exprP = new Pointer<ASTnode>();
            Pointer<ASTnode> varElementSibP = new Pointer<ASTnode>();
            ...
            if(IndiceRep() & FuncOrAssignStatIdnestVarTail()){
                ...
                ASTnode indexList = ASTnodeFactory.makeNode(NodeType.indexList);
                if(exprP.ref!=null){
            ...
            Pointer<ASTnode> fCallParamsP = new Pointer<ASTnode>();
            Pointer<ASTnode> varElementSibP = new Pointer<ASTnode>();
            semStack.push(varElementSibP);
    ...
    private boolean Function(){
        ...
        if(first(Symbol.FuncHead).contains(lookahead.getType())){
            Pointer<ASTnode> funcBodyP = new Pointer<ASTnode>();
    ...
    private boolean Inherit(){
        ...
        if(lookahead.getType().equals(TokenType.Inherits)){
            Pointer<Token> idToken = new Pointer<Token>();
            Pointer<ASTnode> nestedIdP = new Pointer<ASTnode>();
    ...
    private boolean MethodBodyVar(){
        ...
        if(lookahead.getType().equals(TokenType.Var)){
            Pointer<ASTnode> varDeclP = new Pointer<ASTnode>();
    ...
    private boolean Prog(){
        ...
        if(first(Symbol.ClassDecl).contains(lookahead.getType()) || first(Symbol.FuncDef).contains(lookahead.getType()) || lookahead.getType().equals(TokenType.Main)){  
            Pointer<ASTnode> classDeclListP = new Pointer<ASTnode>(ASTnodeFactory.makeNode(NodeType.classDeclList));
            Pointer<ASTnode> funcDefListP = new Pointer<ASTnode>(ASTnodeFactory.makeNode(NodeType.funcDefList));
            Pointer<ASTnode> mainFuncBodyP = new Pointer<ASTnode>(ASTnodeFactory.makeNode(NodeType.funcBody));
    ...
    private boolean Statement(){
        ...
        if(first(Symbol.FuncOrAssignStat).contains(lookahead.getType())){
            ...
            Pointer<ASTnode> relExprP = new Pointer<ASTnode>();
            Pointer<ASTnode> posStatBlockP = new Pointer<ASTnode>();
            Pointer<ASTnode> negStatBlockP = new Pointer<ASTnode>();
            ...
            Pointer<ASTnode> relExprP = new Pointer<ASTnode>();
            Pointer<ASTnode> statBlockP = new Pointer<ASTnode>();
            ...
            Pointer<ASTnode> varP = new Pointer<ASTnode>();
            semStack.push(varP);
            ...
            derivations.add(String.join(" ", derivTemp));
            if(match(TokenType.Read) & match(TokenType.lpar) & Variable() & match(TokenType.rpar) & match(TokenType.semicol)){
            ...
            derivations.add(String.join(" ", derivTemp));
            if(match(TokenType.Write) & match(TokenType.lpar) & Expr() & match(TokenType.rpar) & match(TokenType.semicol)){
            ...
            derivations.add(String.join(" ", derivTemp));
            if(match(TokenType.Return) & match(TokenType.lpar) & Expr() & match(TokenType.rpar) & match(TokenType.semicol)){
    ...
    private boolean StatementList(){
        ...
        if(first(Symbol.Statement).contains(lookahead.getType())){
            Pointer<ASTnode> statSibP = new Pointer<ASTnode>();
    ...
    private boolean Term(){
        ...
        if(first(Symbol.Factor).contains(lookahead.getType())){
            Pointer<ASTnode> leftFactorP = new Pointer<ASTnode>();
            Pointer<ASTnode> multOpP = new Pointer<ASTnode>();
            Pointer<ASTnode> termP = new Pointer<ASTnode>();
            semStack.push(termP);
    ...
    private boolean TermTail(){
        ...
        derivTemp.remove(derivIndex);
        Pointer<ASTnode> multOpP = semStack.pop();
        Pointer<ASTnode> termP = semStack.pop();
        if(!skipErrors(first(Symbol.TermTail), follow(Symbol.TermTail))){return false;}
        if(first(Symbol.MultOp).contains(lookahead.getType())){
            Pointer<ASTnode> factorP = new Pointer<ASTnode>();
            Pointer<ASTnode> nestedMultOpP = new Pointer<ASTnode>();
            Pointer<ASTnode> nestedTermP = new Pointer<ASTnode>();
    ...
    private boolean VarDeclRep(){
        ...
        if(first(Symbol.VarDecl).contains(lookahead.getType())){
            Pointer<ASTnode> varDeclSibP = new Pointer<ASTnode>();
    ...
    private boolean Variable(){
        ...
        //cheating by using lookahead before match
        if(debug){
        ...
        if(lookahead.getType().equals(TokenType.id)){
            ASTnode id = ASTnodeFactory.makeNode(NodeType.id, lookahead);
            Pointer<ASTnode> dataMemberP = new Pointer<ASTnode>();
            dataMemberP.ref = ASTnodeFactory.makeFamily(NodeType.dataMember, id);
    ...
    private boolean VariableIdnest(){
        //*result is dataMember with id child already. Adds indexList child, potentially without any childs of its own if follow condition
        if(debug){
        ...
        if(first(Symbol.IndiceRep).contains(lookahead.getType()) || first(Symbol.VariableIdnestTail).contains(lookahead.getType())){
            Pointer<ASTnode> exprP = new Pointer<ASTnode>();
            Pointer<ASTnode> nestedDataMemberP = new Pointer<ASTnode>();
    ...
    private boolean VariableIdnestTail(){
        //*result is null, remains so if follow condition, or becomes dataMember with 1 id child
        if(debug){
        ...
        if(lookahead.getType().equals(TokenType.dot)){
            ASTnode id = ASTnodeFactory.makeNode(NodeType.id, lookahead);
            result.ref = ASTnodeFactory.makeFamily(NodeType.dataMember, id);
            semStack.push(result);

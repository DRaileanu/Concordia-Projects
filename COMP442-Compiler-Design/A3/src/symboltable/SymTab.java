...
public class SymTab {
	...
	public SymTabEntry search(String entryname, Kind kind){
		...
		switch(kind){
			...
			break;
			case Var:{//look in inherited classes if in Class scope now or in all possible classes if from global) //TODO why from global?
				for(final SymTabEntry entry : entries) {
					if(entry.kind==Kind.Class) {
						returnvalue = entry.scope.search(entryname, kind);
						if(returnvalue!=null){
	...
	//TODO fix or understand how is it useful
	// public SymTabEntry search(String entryName) {
	// 	SymTabEntry returnvalue = new SymTabEntry();
	// 	boolean found = false;
	// 	for( SymTabEntry rec : entries) {
	// 		if (rec.m_name.equals(entryName)) {
	// 			returnvalue = rec;
	// 			found = true;
	// 		}
	// 	}
	// 	if (!found) {
	// 		if (uppertable != null) {
	// 			returnvalue = uppertable.search(entryName);	
	// 		}
	// 	}
	// 	return returnvalue;
	// }

...
public class Client implements Runnable { 
	...
	/* Implement the code for the run method */
    	// call sendTransactions() if sending Client
    	if (clientOperation.contentEquals("sending")) {
    		sendClientStartTime = System.currentTimeMillis();
    		sendTransactions();
    		sendClientEndTime = System.currentTimeMillis();
    		System.out.println("\n Terminating client sending thread - Running time " + (sendClientEndTime - sendClientStartTime) + " millisecond");
    	}
    	//call receiveTransactions() if receiving Client
    	else if(clientOperation.equals("receiving")) {
    		receiveClientStartTime = System.currentTimeMillis();
    		receiveTransactions(transact);
    		receiveClientEndTime = System.currentTimeMillis();
    		System.out.println("\n Terminating client receive thread - Running time " + (receiveClientEndTime - receiveClientStartTime) + " millisecond");
    	}
    	//when both buffers are empty, Client can disconnect, yield until then
    	while (!(objNetwork.getOutBufferStatus().equals("empty") && objNetwork.getInBufferStatus().equals("empty"))){Thread.yield();}
